...
app.get('/', (req, res) => {
  res.send('ASTRA Full System (Tickets + Moderation) is running!');
});
...
// ID قناة اللوقز لكل الأوامر والتذاكر
...
client.on('messageCreate', async (message) => {
    ...
    const targetMember = referencedMsg.member;
...
// -------------------------------------------------------------
// 2. التعامل مع الأزرار والقوائم (التذاكر + الإسكات)
// -------------------------------------------------------------
client.on('interactionCreate', async (interaction) => {
    ...
    if (interaction.customId === 'ticket_select') {
        ...
        try {
            ...
            await channel.send({
                content: `${interaction.user} | ${assignedAdminText}`,
                embeds: [ticketEmbed],
            ...
            await sendLog(
                interaction.guild, 
                '🎫 تكت جديد — ' + ticketTypeName, 
                `**القناة:** ${channel}\n**العضو:** ${interaction.user}\n**الإداري المكلف:** ${assignedAdminText}`,
                '#00ff00'
            );
...
async function closeTicket(channel, user, interaction = null) {
    ...
    await sendLog(
        channel.guild, 
        '🔒 تكت مغلق', 
        `**اسم القناة:** ${channel.name}\n**تم إغلاقه بواسطة:** ${user} (${user.username})`,
        '#ff0000'
    );
